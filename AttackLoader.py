...
class AttackLoader():
  def __init__(self, args, logger, data_type="standard"):
    ...
    if data_type == "standard":
      self.root = "attack-log/original"
      self.data_dir = os.path.join(self.root, args.dataset)
      self.model_dir = os.path.join(self.data_dir, args.model_type)
      self.csv_dir = os.path.normpath(os.path.join(self.model_dir, args.attack_type))
      csv_files = glob.glob(os.path.normpath(os.path.join(self.csv_dir, "*.csv")))
      print(os.path.abspath(self.csv_dir))
