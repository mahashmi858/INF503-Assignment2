...
def split_dataset(dataset, split='trainval', split_ratio=0.8):
    ...
    if num_samples == 0:
        raise ValueError("Dataset is empty. Ensure the dataset is correctly loaded.")

    indices = np.random.permutation(range(num_samples))
    if split == 'trainval':
        train_idx = indices[:int(num_samples * split_ratio)]
        val_idx = indices[int(num_samples * split_ratio):]
        
        if len(train_idx) == 0 or len(val_idx) == 0:
            raise ValueError("Insufficient samples for splitting. Adjust split ratio or dataset size.")
